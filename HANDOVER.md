...
- **To-Do List + Timesheet side panels, plus their drag/resize widget layout (2026-08-18): implemented and deployed as plugin 0.4.28, live on `controll.co.za`.** New per-user, cloud-synced to-do list (grouped Today/This Week, optional priority/due date) and timesheet (start/stop clock, weekly progress bar, today's session log) flanking the Dashboard on `lg:`+ screens, plus drag-to-reposition/resize for both (snapped to a 24px row grid, can't overlap each other or the center Dashboard block). Single-user only — a mocked-up "assignee avatar" idea from the design canvas was explicitly dropped per the user's own scoping call. Rides the existing `workspace` document/sync machinery — no new DB table, no `LINKFLOW_DASHBOARD_DB_VERSION` bump, three new optional top-level keys (`todos`, `timesheet`, `panelLayout`) validated in `sanitize_workspace()`. See the two "2026-08-18" architectural-decision entries below for the full design rationale, and the plugin/desktop READMEs for field-level detail.
  ...
  - **Desktop installer not rebuilt/republished.** Only the WordPress plugin was deployed. The shared React code (already containing this whole feature) is live for anyone running from source or a fresh `build:desktop`, but the existing packaged desktop 0.1.10 installer predates this feature and won't have it until a new signed release ships.
...
- Compatible pairing: desktop **0.1.10** pairs with plugin **0.4.29** (live). The Daily Inspiration bubble needs plugin 0.4.18+ for live data (it degrades gracefully to "unavailable" against older plugin versions that lack the endpoint). Desktop **0.1.3 or later** requires plugin **0.4.9 or later** generally. The REST contract dates to plugin 0.4.5, so a site already running 0.4.5-0.4.8 still responds correctly, but no package before 0.4.9 should be deployed: each has a known install or activation defect on Linux hosts.
...
- **Updated (2026-08-18): To-Do List + Timesheet side panels + drag/resize widget layout — plugin deployed, drag/resize confirmed by hand, but the actual cloud round-trip is still unconfirmed.** What's actually done: plugin 0.4.28 is live (`wp plugin list` confirms `active`, zero schema drift confirmed via `wp option get`/`SHOW TABLES`); drag-between-columns, drag-reorder-within-a-column, and resize-with-snap were all tested by hand in the dev server and work (two real bugs found and fixed this way — see the architectural-decision entry above). Still needed before this can be called fully done:
  ...
  - Build and publish a new signed desktop release (version bump across `package.json`/`Cargo.toml`/`tauri.conf.json`, `tauri build`, GitHub Release) so the already-installed 0.1.10 client actually gets this feature — right now it only exists for anyone running from source or a fresh `build:desktop`.
