...
Current desktop version: **0.1.17**, built and signed but not yet published as a GitHub Release (0.1.16 remains what an in-app update check will currently offer). Requires WordPress plugin **0.4.13 or later** for Google sign-in and desktop-initiated sign-out to work (0.4.30 is the live release; 0.4.31 is packaged alongside 0.1.17 but not yet deployed). Plugin 0.4.26+ additionally required for the account dropdown to show the signed-in user's email (degrades gracefully to just the display name against older plugin versions).
...
**Status (2026-08-19): desktop 0.1.17 built and signed locally; plugin 0.4.31 packaged (version-only, no PHP change). Neither has been published/deployed yet** — 0.1.16 (GitHub Release) and plugin 0.4.30 remain what's actually live/offered by the update proxy. 0.1.16 itself was fully confirmed end-to-end: each of 0.1.12 through 0.1.16 was verified live via the update proxy (`current_version=<previous>` → 200 with a valid signed manifest; `current_version=<new>` → 204; download endpoint streams the real `.exe`) after clearing the WordPress-side `linkflow_latest_github_release` transient, and an already-installed client picked each release up automatically via the in-app updater. A real authenticated cloud save/load round-trip, and everything in the earlier feature batch (manual entry, Today progress bar, wording, Copy button, drag/resize), have all been confirmed working by hand. One real bug was found this way in that earlier batch (an older client's save silently wiping newer fields for a shared account) and fixed in plugin 0.4.30 — see `HANDOVER.md` for the full writeup. **0.1.17's own new content (the What's New tour and the two polish fixes) has not yet been hands-on verified or released** — ask before publishing.
...
### Version 0.1.17 (2026-08-19, with plugin 0.4.31 — packaged, not yet published/deployed)
...
- Built and signed locally (`tauri build --target x86_64-pc-windows-msvc`); WordPress plugin packaged via `package.ps1`. **Neither has been published as a GitHub Release / deployed to `controll.co.za` yet, and the new tour has not been visually verified in a running app** — ask before doing either.
