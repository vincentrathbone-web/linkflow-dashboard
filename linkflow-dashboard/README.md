...
Current desktop version: **0.1.11**. Requires WordPress plugin **0.4.13 or later** for Google sign-in and desktop-initiated sign-out to work (0.4.30 is the paired, live release). Plugin 0.4.26+ additionally required for the account dropdown to show the signed-in user's email (degrades gracefully to just the display name against older plugin versions).
...
**Status (2026-08-19): desktop 0.1.11 built, signed, and published as a GitHub Release, paired with the live plugin (0.4.30).** Confirmed end-to-end via the update proxy: `current_version=0.1.10` → 200 with a valid signed manifest pointing at the new installer; `current_version=0.1.11` → 204; the download endpoint streams the real `.exe`. An already-installed 0.1.10 client picks this up automatically via the in-app updater — confirmed live by the user's own update. A real authenticated cloud save/load round-trip, and everything in the feature batch (manual entry, Today progress bar, wording, Copy button, drag/resize), have all now been confirmed working by hand. One real bug was found this way (an older client's save silently wiping newer fields for a shared account) and fixed in plugin 0.4.30 — see `HANDOVER.md` for the full writeup.
